...
function $$(expr, con) {
  const elements = (con || document).querySelectorAll(expr);

  try {
    return Array.prototype.slice.call(elements);
  } catch (e) {
    const arr = Array(elements.length);
...
(function () {
  var _ = (window.Utopia = {
    ...
    each: function (obj, func, context) {
      ...
      for (const i in obj) {
        if (obj.hasOwnProperty && obj.hasOwnProperty(i)) {
          const ret = func.call(context, obj[i], i);
     ...
     * When there is a collision, the first object WinRS
     */
    ...
    event: {
      ...
      bind: function (target, event, callback, traditional) {
        if (_.type(target) === "string" || _.type(target) === "array") {
          const elements = _.type(target) === "string" ? $$(target) : target;
